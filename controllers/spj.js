...
spj.setRedisClient = (client)=>{
	redisClient = client;
}
var getLoggedUser = require('./function/getLoggedUser')

spj.socket = function(io, connections, client, loggedUser){
...
spj.get('/honor', function(req, res){
	getLoggedUser( redisClient, req.cookies.uid, ( loggedUser ) => {
		Setting.findOne({type: 'spj'}).exec(function(err, result){
			console.log(result)
			if(result){
				res.render('spj/honor', {layout: false, admin: loggedUser.jenis, daftar: result.get('honor_daftar'), honor_detail_id: result.get('honor_detail_id'), pembuat_daftar_honor: result.get('pembuat_daftar_honor')});
			} else {
				res.render('spj/honor', {layout: false, admin: loggedUser.jenis});
			}
		})
	})
});
...
spj.post('/honor', function(req, res){
	getLoggedUser( redisClient, req.cookies.uid, ( loggedUser ) => {
		...
		async.series([
				...
				if(err){
					sendNotification(req.cookies.uid, err);
					res.sendStatus(404)
					return;
				}
				...
				.then(workbook => {
					...
					async.series([
							...
							async.series([
								...
								DetailBelanja.findOne({'thang': thang, '_id': honor_detail_id, active: true}, 'realisasi').elemMatch('realisasi', {'jumlah': getNumber(data[i]['bruto']), 
									...
									if(!result){
										...
										var new_entry = {};
										new_entry.pengentry = loggedUser.username;
										...
										DetailBelanja.update({'thang': thang, "_id": honor_detail_id}, {$push: {"realisasi": new_entry}}, {new: true}, function(err, result){
											...
											sendNotification(req.cookies.uid, 'Honor '+data[i]['nama']+' berhasil tercatat di realisasi.')
										})
									} else {
										sendNotification(req.cookies.uid, 'Honor '+data[i]['nama']+' periode tsb sudah tercatat di realisasi.')
									}
								})
							})
						})
						...
						//riwayat user
						User.update({_id: req.cookies.uid}, {$push: {"act": {label: 'Buat SPJ Honor Dosen periode '+periode}}}, 
							function(err, status){
						})
					})
				})
...
spj.get('/transport', function(req, res){
	getLoggedUser( redisClient, req.cookies.uid, ( loggedUser ) => {
		Setting.findOne({type: 'spj'}).exec(function(err, result){
			if(result){
				res.render('spj/transport', {layout: false, admin: loggedUser.jenis, daftar: result.get('transport_daftar'), transport_detail_id: result.get('transport_detail_id'), pembuat_daftar_transport: result.get('pembuat_daftar_transport')});
			} else {
				res.render('spj/transport', {layout: false, admin: loggedUser.jenis});
			}
		})
	} );
});
		...
		async.series([
				...
				.then(workbook => {
					...
					async.series([
								...
								DetailBelanja.findOne({'thang': thang, '_id': transport_detail_id, active: true}, 'realisasi').elemMatch('realisasi', {'jumlah': getNumber(data[i]['jumlah'] * 150000), 
									'tgl': tgl_buat_honor, 'penerima_id': penerima_id}).exec(function(err, result){
										...
										if(!result){
											...
											sendNotification(req.cookies.uid, 'Transport tsb untuk '+data[i]['nama']+' pernah tercatat di realisasi.')
										}
								})
							})
						})
						...
						// //riwayat user
						User.update({_id: req.cookies.uid}, {$push: {"act": {label: 'Buat SPJ Transport Dosen Non STIS periode '+periode}}}, 
							function(err, status){
						})
					})
				})
...
spj.get('/spj_lainnya', function(req, res){
	getLoggedUser( redisClient, req.cookies.uid, ( loggedUser ) => {
		res.render('spj/lainnya', {layout: false, admin: loggedUser.jenis});
	} );
});
